...
x %>%
  ggplot(aes(y = urn_red, x = paddle_red)) + geom_point(alpha = 0.5)
```
...
post <- x %>%
  ...
  unnest(new_reds) %>%
  mutate(res = ifelse(new_reds > 3, 1, 0), weighted_res = res*probs) %>%
  summarize(total_prob = sum(weighted_res))
```
...
```{r}
probabilities2 <- tibble(urn_red = 0:1000) %>% 
  mutate(paddle_red = map(urn_red, ~ rbinom(n = 100, size = 20, prob = ./1000))) %>%
  unnest(paddle_red) %>%
  mutate(res = ifelse(paddle_red > 3, 1, 0)) %>%
  mutate(weighted_res = res*probs)
